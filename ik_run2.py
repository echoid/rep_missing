...
def main():
    ...
    for fill_type in ["zero","mean","mice"]:
        ...
        for fold_n in index.keys():
            ...
            print("Im using X_combined")
            component_mat =  IKSimilarity(X_combined)
            #component_mat_test =  IKSimilarity(X_test_imputed)

            svm = SVC(C=C, kernel='precomputed')
            svm.fit(component_mat, y_combined)
            y_pred = svm.predict(component_mat)
