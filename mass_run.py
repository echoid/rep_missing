...
def main():
    ...
    data_stats = None # need to update
    ...
    for fill_type in ["zero","mean","mice"]:
        ...
        for fold_n in index.keys():
            ...
            train, test = m0_krn.build_model(X_train_imputed, X_test_imputed)  # this does the pre-processing step

            print("- Sim: Train")
            train_sim = m0_krn.transform(train)

            print("- Sim: Train/Test")
            test_sim = m0_krn.transform(test,train)  # row = train, col = test
    ...
    csv_file_name = "results/{}/{}/{}/{}.csv".format("Mass", dataname, mech, para)
